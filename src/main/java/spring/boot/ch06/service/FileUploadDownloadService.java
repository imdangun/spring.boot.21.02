package spring.boot.ch06.service;
...
import spring.boot.ch06.domain.FileDownloadException;
import spring.boot.ch06.domain.FileUploadException;
...
public class FileUploadDownloadService {
	...
	public FileUploadDownloadService(FileUploadProperties prop) {
		...
		try {
			...
			throw new FileUploadException("파일 업로드 디렉토리를 생성하지 못했습니다.");
		}
	}
	...
	public Resource loadFileAsResource(String fileName) {
		try {
			...
			if(resource.exists())
				return resource;
			else throw new FileDownloadException(fileName + " 파일을 찾을 수 없습니다.");
